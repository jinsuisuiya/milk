...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    if (typeof updateMessageRecallUI === 'function') updateMessageRecallUI();
    updateAutoSendUI();
    updateDelayUI();
...
const recallToggle = document.getElementById('message-recall-toggle');
...
const recallValue = document.getElementById('message-recall-value');

window.updateMessageRecallUI = function() {
    if (!recallToggle || !recallControl || !recallSlider || !recallValue) return;
    recallToggle.classList.toggle('active', !!settings.messageRecallEnabled);
    recallControl.style.display = settings.messageRecallEnabled ? "flex" : "none";
    const pct = Math.round((settings.messageRecallChance !== undefined ? settings.messageRecallChance : 0.10) * 100);
    recallSlider.value = pct;
    recallValue.textContent = `${pct}%`;
};

if (recallToggle) {
    updateMessageRecallUI();

    recallToggle.addEventListener('click', () => {
        settings.messageRecallEnabled = !settings.messageRecallEnabled;
        updateMessageRecallUI();
        throttledSaveData();
        showNotification(`随机撤回已${settings.messageRecallEnabled ? '开启' : '关闭'}`, 'success');
    });

    recallSlider.addEventListener('input', (e) => {
        const val = Math.max(5, Math.min(15, parseInt(e.target.value)));
        settings.messageRecallChance = val / 100;
        recallValue.textContent = `${val}%`;
    });
